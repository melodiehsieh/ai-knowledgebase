...
def Timer(name):
    ...
    print(f"{text_format.BOLD}{name} took {duration} seconds{text_format.ENDC}")
...
async def queries(qual_instructions, qual_context, question, quant_sql_instructions, data_instructions, quant_snowflake_context, embed_model):
    ...
    with Timer("Claude Qual + SQL Generation"):
    ...
    with Timer("Run Snowflake query"):
    ...
    with Timer("Claude quant analysis"):
    ...
    cost_tracker.print_summary([qual_usage, quant_usage, snowflake_query_usage])
...
async def main():
    ...
    data_instructions = "You are assisting [redacted], a network automation and orchestration company, with data-oriented questions they have about company operations. " \
